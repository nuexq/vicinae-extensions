...
export default function SearchListItem({
	...
	const safeName = searchResult.name
		.replace(/</g, "&lt;")
		.replace(/>/g, "&gt;");
	const markdown = `## ${safeName}\n\n${searchResult.description ? getGitHubUrl ? searchResult.description : turndownService.turndown(searchResult.description) : ""}\n\n${searchResult.example ? `### Example\n\`\`\`nix\n${searchResult.example}\n\`\`\`` : ""}`; // getGithubUrl pass only by home-manager options command
